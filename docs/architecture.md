...
Система поддерживает три несвязанных типа соревнования, хранящихся в поле
`competitions.type`:
...
| `firm` | «Фирменное», 10 пар / 20 стрелков (щиты 1–20), группировка по щитам | 2 строки на щит | `s1,s2` | `5 5` |
| `final` | индивидуальный финал, 8 стрелков, без пар | 1 строка на стрелка | `s1..s9` | `5 5 2 2 2 2 2 2 2` |

Парная группировка (заголовок «Пара», разделители, сумма по паре) включается для
`pairs` и `firm` через общий предикат `isPairBasedType(type)`. Для `firm` read-path
учитывает только `s1,s2` — `s3..s9` игнорируются (и не заполняются).
